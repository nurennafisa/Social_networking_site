...
$_SESSION['searchname']='$_POST[search]';
?>
<?php
   if(isset($_POST['submit-search'])){
	   $search=mysqli_real_escape_string($conn,$_POST['search']);
	   $sql="SELECT * FROM sign_up WHERE username LIKE '%$search%' OR firstname LIKE '%$search%' OR  lastname LIKE '%$search%' 
	   ...
	   if($queryResult>0){
		   while($row=mysqli_fetch_assoc($result)){ 
			   ...
			   echo "<a href='http://localhost/Amidstus/search_result.php?title=".$row['username']."'><div class='user-box'>
		  <h3> ".$row['username']."</h3>
		  <p>".$row['firstname']."</p>
		  <p>".$row['lastname']."</p>
