...
?> 
    ...
    <div class="w3-col m7">
      ...
      <h3><center>CHECK PROFILE BEFORE BECOMING FRIENDS!</center></h3>	  
            </div>
...
mysql_select_db("amidstus",$con);
$result = mysql_query("SELECT * FROM user_profile
WHERE user_id=$_SESSION[searchid] ");
$result1 = mysql_query("SELECT * FROM sign_up
WHERE user_id= $_SESSION[searchid]");


$_SESSION['request']=$_SESSION['user_id'];
$_SESSION['requestto']= $_SESSION['searchid'] ;
...
{  
echo "USERNAME:  ". $row['username'];echo "<br/><br/>";
echo "EMAIL:  ". $row['email'];echo "<br/><br/>";
}
while($row = mysql_fetch_array($result))
{  

echo "WORKPLACE:". $row['workplace'];echo "<br/><br/>";
echo "EDUCATIONAL INSTITUTION:". $row['educational_institution'];echo "<br/><br/>";
echo "DATE OF BIRTH:". $row['dob'];echo "<br/><br/>";
echo "MOTTO OF YOUR LIFE:". $row['motto'];echo "<br/><br/>";
  } 

mysql_close($con);

?> 	
<a href="add_friend.php" value="MAKE A FRIEND REQUEST">MAKE A FRIEND REQUEST</br>	</a>
<a href="amidstus.php" value="MAKE A FRIEND REQUEST">IGNORE	</a>
